        ...
        def kernel(self, a: str):
            ...
            while self.running:
                ...
                if aboutPage:
                    ...
                    startPage = True
                pygame.display.update()
def preparation(inputArgs:list)->None:
    inputArg = inputArgs[1]
    ...
    if inputArg == '-e':
        ...
        from create_chapter import chapter
        self = chapter()
        chapter.main(self)
        quit()
    elif inputArg == '-h' or inputArg == '--help':
        print('''Thank you for using Plane War!
        Powered By Pygame and tkinter
        CLI Commands:
        ...
        themePath = inputArgs[2]
        if not (themePath in os.listdir("./themes")):
            print("InValid Theme Name,Turn To default Theme ")
        else:
            import json
            settings = json.loads(tools.file.readFile(
                "./data/setting.json", 0, 100))
            settings["themePath"] = f"./themes/{themePath}"
            tools.file.writeFile("settings.json",json.dumps(settings))
...
if __name__ == '__main__':
    try:
        sys.argv[1]
        preparation(sys.argv)
    finally:
        self = programme()
        init.init(self)
        self.chapter = False

        if start.detect_chapters(self):
            answer = restorer.askChapter(self)
            if type(answer) == int:
                self.chapter = True
        ...
        self.screen = pygame.display.set_mode(
            (tools.file.config("screenWidth"), tools.file.config("screenHeight")))
